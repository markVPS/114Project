...
PCB::PCB(int pid_, int arrival_, int burst_, int memory_, int priority_) //PCB is able to track the requests/allocation of memory
    : pid(pid_),
...
std::string stateToString(ProcessState state) { // converts the enum value of a procees state to a string
    switch (state) {
